...
test_that('nelson', {
    a <- nelson(algebra('luk'))
    expect_that(a$algebratype, equals(c('lukasiewicz', 'nelson')))


    a <- nelson(algebra('gog'))

    expect_that(a$algebratype, equals(c('goguen', 'nelson')))
    ...
    expect_that(a$r(c(0.5, 0.8,   NA,      0.3,       NA,   0, NA, NA),
             ...
             equals(c(1.0, 0.625, NA_real_, NA_real_,  1,   1,  1,  1)))

    expect_that(a$b(c(0.5,   0.8,   NA,       0.4,      NA,  0,        1),
                    c(0.8,   0.5,   0.4,      NA,       NA, NA,       NA)),
             equals(c(0.625, 0.625, NA_real_, NA_real_,  1,  1, NA_real_)))
...
test_that('nelson is non-associative', {
    a <- nelson(algebra('lukasiewicz'))
