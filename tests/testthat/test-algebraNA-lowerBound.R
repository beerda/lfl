...
test_that('lowerBound', {
    a <- lowerBound(algebra('luk'))
    expect_that(a$algebratype, equals(c('lukasiewicz', 'lowerBound')))


    a <-lowerBound(algebra('gog'))

    expect_that(a$algebratype, equals(c('goguen', 'lowerBound')))
    ...
    expect_that(a$r(c(0.5, 0.8,   NA,      0.3,       NA,    0,    1,    NA, NA, 0,   1, 0.6),
             ...
             equals(c(1.0, 0.625, 1.0,   NA_real_,     1,    1, NA_real_, 0, 1,  1,  0.3, 0)))

    expect_that(a$b(c(0.5,   0.8,   NA,       0.4,      NA,       0,        1),
                    c(0.8,   0.5,   0.4,      NA,       NA,       NA,       NA)),
             equals(c(0.625, 0.625, NA_real_, NA_real_,  1,       0,        NA_real_)))
...
test_that('lowerBound 2', {
    ...
    expect_equal(a$r(NA, 0.5), 1)
    expect_equal(a$r(NA, 1), 1)
    expect_equal(a$r(NA, NA), 1)

    a <- lowerBound(algebra('lukasiewicz'))
    expect_equal(a$r(0.5, NA), NA_real_)
})


test_that('lowerBound order', {
    a <- lowerBound(algebra('lukasiewicz'))
