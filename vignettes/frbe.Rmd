...
train_ts <- window(realistic_ts, end=c(9, 12))  # First 9 years
test_ts <- window(realistic_ts, start=c(10, 1))  # Last year

# Fit on training data
fit <- frbe(train_ts, h=12)

# Evaluate against test data
evaluation <- evalfrbe(fit, test_ts)
print(evaluation)
```
...
for (i in 1:n_test) {
  ...
  train <- window(data, end=c(1, n_train + i - 1))
...
train <- window(realistic_ts, end=c(9, 12))
test <- window(realistic_ts, start=c(10, 1))
